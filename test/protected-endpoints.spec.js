...
const { testUsers, testReviews, testThings } = helpers.makeThingsFixtures()
...
describe('Protected endpoints', () => {
  ...
  context('user is authenticated', () => {
    ...
    it('GET /api/things/:thing_id/reviews returns 200 and expected response', () => {
      const thingId = 1
      const expectedThingReviews = helpers.makeExpectedThingReviews(
        testUsers,
        thingId,
        testReviews
      )
    ...
    it('POST /api/reviews returns 201', () => {
      const newReview = {
        thing_id: 1,
        rating: 4,
        text: 'new and exciting review',
      }
      return db
        .raw(
          `SELECT setval('thingful_reviews_id_seq', ${testReviews[testReviews.length - 1].id})`
        )
        .then(() => {
          return supertest(app)
            .post('/api/reviews')
  ...
  context('user is not authenticated', () => {
    it('GET /api/things/:thing_id returns 401 and expected response with bad jwt secret', () => {
      return supertest(app)
        ...
        .set(
          'Authorization',
          `Bearer ${helpers.makeAuthHeader(testUsers[0], 'bad-token')}`
        )
